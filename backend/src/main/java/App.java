...
public class App {
    ...
    private static Connection connect() throws Exception {
        for (int i = 0; i < 60; i++) {
            try {
                return DriverManager.getConnection("jdbc:mysql://mysql-db/example?allowPublicKeyRetrieval=true&useSSL=false", "root", Files.lines(Paths.get("/run/secrets/db-password")).findFirst().get());
